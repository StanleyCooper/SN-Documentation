...
//Put the role that user needs to have here
data.new_portal_home = gs.hasRole('test_restricted_access');

if (data.new_portal_home) {
...
function($rootScope, $scope, snRecordWatcher, spUtil, $location, $uibModal, cabrillo, $timeout, $window) {
    ...
    //handles redirect if user doesn't have role - if nUrl = False
    var c = this;
    var nUrl = c.data.new_portal_home;
    if (!nUrl) {
		//the alert message to display
        alert('You currently do not have permission to view this portal. You will be redirected back to the main service portal.');
        //the URL to direct to
		$location.url('/isu');

    }
	//if nURL is true user has that role.
	else {
