(function process(g_request, g_response, g_processor) {
    ...
    grKK.addEncodedQuery("kb_knowledge_base=378e51056f46c200f290bc226e3ee44a^can_read_user_criteriaISEMPTY^workflow_state=published");
    grKK.query();
...
by KEVIN CUSTER on JULY 28, 2020
ServiceNow
Intro
For organizations having public-facing service portals, there is often a need to have public content indexed, or crawled, by search engines in order for users to quickly and easily find relevant articles. Building a sitemap is one way of improving SEO by essentially announcing a given list of links that search crawlers should visit. I will walk you through setting this up in ServiceNow by using a custom Processor. We will be using the sitemaps.org protocol, so feel free to visit the link and get familiar with the XML tags to use.

Note: You will need the Custom Search Integration plugin activated in your instance, which must be done through a HI request, in order to update Robots.txt - this is necessary for this guide.

Create a Processor
Disclaimer: Per ServiceNow's documentation, processors are in fact a deprecated feature. However, they can still be created and used, at least for now. In the future I will amend this article to include instructions for using the recommended replacement, which is Scripted REST APIs.
...
Modify the Form Layout to include the Roles Field.
Enter the following field values for the Processor:
Field	Value
...
Path	sitemap
Modify the script below to suit your needs, and enter it in the Script field. The comments in the script describe what is being done, so be sure to read them! :) :
(function process(g_request, g_response, g_processor) {
...
In the navigator type sys_public.list and press enter.
Click new, and enter the following field values:
Field	Value
Page	sitemap
Active	true (checked)
This allows us to navigate to https://.service-now.com/sitemap.do without being logged in. Go ahead and try it in a private browser window! You should see something like this if everything works as expected:

ServiceNow Sitemap Results
Update Robots.txt
...
Copyright © 2021 Kevin Custer. All rights reserved.

RSS | Sitemap

//OG WESLYAN ROBOTS DONT Use*
Disallow: /404
Disallow: /index
Disallow: /landing
Disallow: /login
Disallow: /offering
Disallow: /public_survey
Disallow: /search
